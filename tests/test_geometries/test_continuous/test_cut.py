...
def test_create_cut_domain_2d(backend):
    parallel = Parallelogram(Y, [0, 0], [1, 0], [0, 1], backend=backend)
    circle = Circle(Y, [0, 0], 0.5, backend=backend)
    cut_domain = parallel - circle

    assert isinstance(cut_domain, CutGeometry)
    assert cut_domain.variable == Y
    assert cut_domain.backend == backend
    assert cut_domain.geometry_a == parallel
    assert cut_domain.geometry_b == circle


@pytest.mark.parametrize("backend", BACKENDS)
def test_create_cut_domain_3d(backend):
    box = Box(X, [0, 0, 0], 1.0, 2.0, 1.0, backend=backend)
    sphere = Sphere(X, [0, 0, 0], 0.5, backend=backend)
    cut_domain = box - sphere

    assert isinstance(cut_domain, CutGeometry)
    assert cut_domain.variable == X
    assert cut_domain.backend == backend
    assert cut_domain.geometry_a == box
    assert cut_domain.geometry_b == sphere


@pytest.mark.parametrize("backend", BACKENDS)
def test_contains_cut_domain_2d(backend):
    parallel = Parallelogram(Y, [0, 0], [1, 0], [0, 1], backend=backend)
    circle = Circle(Y, [0, 0], 0.5, backend=backend)
    cut_domain = parallel - circle

    inside = cut_domain.contains([[0.8, 0.2]])
    assert backend.math.all(inside)

    outside = cut_domain.contains([[1.1, 0.5], [0.2, 0.2]])
    expected = backend.build_tensor([[False], [False]])
    assert backend.math.all(outside == expected)


@pytest.mark.parametrize("backend", BACKENDS)
def test_contains_cut_domain_3d(backend):
    box = Box(X, [0, 0, 0], 1.0, 2.0, 1.0, backend=backend)
    sphere = Sphere(X, [0, 0, 0], 0.5, backend=backend)
    cut_domain = box - sphere

    inside = cut_domain.contains([[0.8, 0.5, 0.5]])
    assert backend.math.all(inside)

    outside = cut_domain.contains([[1.1, 0.5, 0.5], [0.2, 0.1, 0.1]])
    expected = backend.build_tensor([[False], [False]])
    assert backend.math.all(outside == expected)
...
def test_sample_random_uniform_cut_2d(backend, device):
    parallel = Parallelogram(Y, [0, 0], [1, 0], [0, 1], backend=backend)
    circle = Circle(Y, [0, 0], 0.5, backend=backend)
    cut_domain = parallel - circle

    points = cut_domain.sample_random_uniform(20, device=device)
    assert points.shape == (20, 2)
    assert backend.math.all(cut_domain.contains(points))
...
@pytest.mark.parametrize("backend", BACKENDS)
@pytest.mark.parametrize("device", devices)
def test_sample_random_uniform_cut_3d(backend, device):
    box = Box(X, [0, 0, 0], 1.0, 2.0, 1.0, backend=backend)
    sphere = Sphere(X, [0, 0, 0], 0.5, backend=backend)
    cut_domain = box - sphere
...
@pytest.mark.parametrize("backend", BACKENDS)
@pytest.mark.parametrize("device", devices)
def test_sample_grid_cut_2d(backend, device):
    parallel = Parallelogram(Y, [0, 0], [1, 0], [0, 1], backend=backend)
    circle = Circle(Y, [0, 0], 0.5, backend=backend)
    cut_domain = parallel - circle

    pts = cut_domain.sample_grid(16, device=device)
    assert pts.shape == (16, 2)
    assert backend.math.all(cut_domain.contains(pts))


@pytest.mark.parametrize("backend", BACKENDS)
@pytest.mark.parametrize("device", devices)
def test_sample_grid_cut_3d(backend, device):
    box = Box(X, [0, 0, 0], 1.0, 2.0, 1.0, backend=backend)
    sphere = Sphere(X, [0, 0, 0], 0.5, backend=backend)
    cut_domain = box - sphere

    pts = cut_domain.sample_grid(24, device=device)
    assert pts.shape == (24, 3)
    assert backend.math.all(cut_domain.contains(pts))


@pytest.mark.parametrize("backend", BACKENDS)
def test_boundary_contains_cut_2d(backend):
    parallel = Parallelogram(Y, [0, 0], [1, 0], [0, 1], backend=backend)
    circle = Circle(Y, [0, 0], 0.5, backend=backend)
    boundary = (parallel - circle).boundary

    assert backend.math.all(boundary.contains([[1.0, 0.5], [0.6, 0.0]]))
    assert float(boundary.contains([[0.2, 0.2]]).reshape(-1, 1)[0][0]) == 0.0


@pytest.mark.parametrize("backend", BACKENDS)
def test_boundary_contains_cut_3d(backend):
    box = Box(X, [0, 0, 0], 1.0, 2.0, 1.0, backend=backend)
    sphere = Sphere(X, [0, 0, 0], 0.5, backend=backend)
    boundary = (box - sphere).boundary

    assert backend.math.all(boundary.contains([[1.0, 0.5, 0.5], [0.6, 0.0, 0.0]]))
    assert float(boundary.contains([[0.2, 0.1, 0.1]]).reshape(-1, 1)[0][0]) == 0.0


@pytest.mark.parametrize("backend", BACKENDS)
@pytest.mark.parametrize("device", devices)
def test_boundary_random_sampling_cut_2d(backend, device):
    parallel = Parallelogram(Y, [0, 0], [1, 0], [0, 1], backend=backend)
    circle = Circle(Y, [0, 0], 0.5, backend=backend)
    boundary = (parallel - circle).boundary
...
@pytest.mark.parametrize("backend", BACKENDS)
@pytest.mark.parametrize("device", devices)
def test_boundary_random_sampling_cut_3d(backend, device):
    box = Box(X, [0, 0, 0], 1.0, 2.0, 1.0, backend=backend)
    sphere = Sphere(X, [0, 0, 0], 0.5, backend=backend)
    boundary = (box - sphere).boundary
