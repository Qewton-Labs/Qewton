...
from ..base import ContinuousGeometry
...
class UnionGeometry(ContinuousGeometry):
    ...
    def __init__(
        self, domain_a: ContinuousGeometry, domain_b: ContinuousGeometry, disjoint=False
    ):
        assert domain_a.variable == domain_b.variable
        assert domain_a.shape == domain_b.shape
        self.domain_a = domain_a
        self.domain_b = domain_b
        self.disjoint = disjoint
        super().__init__(variable=domain_a.variable, shape=domain_a.shape)
...
class UnionBoundaryDomain(BoundaryDomain):
    ...
    def _contains(self, points, params=Points.empty()):
        ...
        if torch.any(on_both):
            ...
            inner_product_ok = (
                torch.sum(normals_a * normals_b, dim=-1, keepdim=True) >= self.overlap_tol
            )
    ...
    def normal(self, points, params=Points.empty(), device="cpu"):
        points, params, device = self._transform_input_for_normals(points, params, device)
