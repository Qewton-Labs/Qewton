...
from ..base import Geometry, BoundaryDomain
...
class Interval(Geometry):
    ...
    def __init__(self, space, lower_bound, upper_bound):
        assert space.dim == 1
        lower_bound, upper_bound = self.transform_to_user_functions(
    ...
    def __call__(self, **data):
        ...
        return Interval(
            space=self.space, lower_bound=new_lower_bound, upper_bound=new_upper_bound
        )
...
class IntervalSingleBoundaryPoint(BoundaryDomain):
    ...
    def __call__(self, **data):
        ...
        return IntervalSingleBoundaryPoint(
            evaluate_domain, side=self.side, normal_vec=self.normal_vec
        )
